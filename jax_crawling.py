...
#
# Promo handling: phrases like "Buy 4 & Get $100 eGift Card" appear BEFORE
# the real price and before the product name, so we strip them out (and capture
# them in `promo`) before doing any price/name extraction.
_JS_EXTRACT = r"""
return (function() {
    var results = [];
    var seen = {};
    ...
    /* Promo phrases that contain a "$NN" amount but are NOT the tyre price.
       Order matters: longer / more specific patterns first. */
    var PROMO_PATTERNS = [
        /BUY\s*4\s*[&+]\s*GET\s*\$([\d,]+(?:\.\d+)?)\s*(?:e[- ]?Gift|gift)\s*card/gi,
        /BUY\s*4\s*[&+]\s*GET\s*\$([\d,]+(?:\.\d+)?)\s*cash\s*back/gi,
        /\$([\d,]+(?:\.\d+)?)\s*(?:e[- ]?Gift|gift)\s*card/gi,
        /\$([\d,]+(?:\.\d+)?)\s*cash\s*back/gi
    ];

    /* Extract promo text + amount, then strip from working copy of text. */
    function extractPromos(text) {
        var promos = [];
        var cleaned = text;
        for (var i = 0; i < PROMO_PATTERNS.length; i++) {
            var pat = PROMO_PATTERNS[i];
            pat.lastIndex = 0;
            var m;
            while ((m = pat.exec(cleaned)) !== null) {
                promos.push(m[0].replace(/\s+/g, ' ').trim());
            }
            cleaned = cleaned.replace(pat, ' ');
        }
        /* Also strip dangling "+1" / "+2" markers and trailing "BUY 4 & GE..." fragments */
        cleaned = cleaned.replace(/\+\d+\s*/g, ' ');
        cleaned = cleaned.replace(/BUY\s*4\s*[&+]\s*GE[A-Z]*\s*/gi, ' ');
    ...
    function extractCard(card) {
        ...
        /* Strip promo phrases BEFORE looking for price/name. */
        var pr = extractPromos(raw);
        var text  = pr.cleaned;
        var promo = pr.promo;

        /* Size: extract from cleaned text */
        var size = '';
        ...
        if (!name) {
            ...
            if (hEl) {
                var hTxt = extractPromos((hEl.textContent || '').replace(/\s+/g,' ').trim()).cleaned;
                hTxt = hTxt.replace(BRAND_NAMES_RE, '').trim();
                if (hTxt) name = hTxt;
            }
        }
        ...
        if (!name) {
            ...
            for (var ei = 0; ei < elems.length; ei++) {
                ...
                if (et.length > 3 && et.length < 60 &&
                        !/\$|\d{3}\/|\bqty\b|booking|detail|view|store|gift\s*card|cash\s*back|buy\s*4/i.test(et) &&
                        !/^\d+/.test(et)) {
                    name = et; break;
                }
            }
        ...
        if (!name) {
            ...
            for (var li = 0; li < ls.length; li++) {
                ...
                if (lt.length > 5 && !/add|booking|detail|view|cart|gift\s*card|cash\s*back|buy\s*4/i.test(lt)) {
                    name = lt; break;
                }
            }
        }
        ...
        if (b4m) {
            ...
            var bulkPromo = 'OR BUY 4 FOR $' + b4m[1];
            promo = promo ? (promo + ' | ' + bulkPromo) : bulkPromo;
        }
        if (!/Buy\s*4\s*Get\s*4th/i.test(promo) &&
                /BUY\s+4\s*[&+]\s*GET\s+4TH\s+TYRE\s+FREE/i.test(raw)) {
            var freePromo = 'Buy 4 Get 4th Free';
            promo = promo ? (promo + ' | ' + freePromo) : freePromo;
            if (price && !disc) disc = (Math.round(parseFloat(price) * 0.75 * 100) / 100).toFixed(2);
        }
