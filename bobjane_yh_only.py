...
bobjane_yh_only.py
------------------
Yokohama-only test variant of bobjane_crawling.py.  Adds two things
the previous crawler missed:

1) LT / off-road size formats.  bobjane_crawling.py's size regex only
   matched standard passenger sizes (315/35R20) and left the SIZE
   column empty for LT tyres like "33/10.5R15" or "35X12.5R18".  This
   variant widens the regex to catch two-digit widths and decimal
   aspect ratios.

2) Size normalisation.  Bob Jane's product titles are inconsistent:
   the same LT shape is sometimes written "33/10.5R15" (slash) and
   sometimes "35X12.5R18" (X).  We normalise the slash-with-decimal
   form to X so downstream matching / price comparison sees a single
   canonical shape — but standard passenger sizes (315/35R20) keep
   their slash because that IS the canonical form there.

Filters output to Yokohama only.  Once the size handling looks right,
roll the same regex + normalisation back into bobjane_crawling.py for
every brand.

Output: BobJane_yh_only_YYYYMMDD_HHMM.csv

Usage:
    python bobjane_yh_only.py
"""
import re
import csv
import time
...
# Bob Jane's Shopify collections support brand-specific URLs, so
# hitting /collections/yokohama loads only Yokohama products and the
# Load More loop reaches the end after ~a couple of clicks — much
# faster than crawling the entire /collections/tyres catalogue and
# post-filtering.  Fall back to a Yokohama post-filter (BRAND_FILTER)
# in case the brand URL 404s on a future site change.
BASE_URL       = "https://www.bobjane.com.au/collections/yokohama"
BRAND_FILTER   = "Yokohama"
OUTPUT_FILE    = datetime.now().strftime("BobJane_yh_only_%Y%m%d_%H%M.csv")
MAX_CLICKS     = 300
...
def write_rows(writer, rows):
    """Writes rows to CSV.  In this Yokohama-only test build, rows whose
    brand isn't Yokohama are silently dropped — a safety net in case the
    /collections/yokohama URL ever starts serving mixed brands."""
    written = 0
    for r in rows:
        if BRAND_FILTER and r["brand"].lower() != BRAND_FILTER.lower():
            continue
...
def main():
    ...
    try:
        ...
        with open(OUTPUT_FILE, "w", newline="", encoding="utf-8-sig") as f:
            ...
            n_written = write_rows(writer, rows)
            f.flush()
            total    += n_written
            print(f"  ── saved {n_written} Yokohama rows (of {len(rows)} raw)  (total: {total})\n")
            ...
            while click_num < MAX_CLICKS:
                ...
                print(f"  Click {click_num:>3}: +{after:>3} cards → {n_written} Yokohama (of {len(rows)} raw)  (total: {total}){tag}")
                if extract_fail >= 5:
