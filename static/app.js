...
async function drawMonthlyKPI(){
  ...
  el.innerHTML = `
  <thead>
  <tr style="font-size:8px; line-height:1.2;">
  ...
  <tbody>
    ${rows.map(r => `
      <tr style="background-color:${rowBg(r.region)}; font-size:8px; line-height:1.2;">
...
async function drawMonthlyTotals(){
  ...
  monthlyInst = new Chart(document.getElementById("monthlyChart"),{
    ...
    data:{ labels, datasets:[
      ...
      // 2026 Target — warm amber + dashed border so it's obviously a
      // projection.  Warm hue contrasts with the slate/blue actuals so
      // all three bars are easy to tell apart at a glance.
      {
        label:"Target (2026)",
        type:"bar",
        data:target26,
...
async function drawMonthlyStacked(){
  ...
  // Promo affects which side stacks make sense:
  //   2025 actual — now has promo_plan coverage (TrueBlue from Jul-25
  //   onwards, 443/iON full year), so it stays in for both Promotion-
  //   group view and promo-button-filtered view; matching sales just
  //   surface alongside 2026.
  //   2026 target — target_26 has no per-promo tagging, so the bucket
  //   would always read 100% Non-Promotion under promo grouping or
  //   100% of the selected promo under filter.  Drop it in both cases.
  const _hasPromo = (typeof window.getActivePromos === "function"
                     && window.getActivePromos().length > 0);
  ...
  // - 2026 target breakdown (skipped when promo logic is in play)
...
// Top-N sold_to panel between KPI table and Profit chart.  Only
// visible while a Top 10/20/30 chip is active — hides otherwise so
// the profit box gets its full width back.
...
async function _refreshTopSoldPanelImpl(){
  ...
  let d;
  try {
    d = await fetch(`/api/top_sold_to_details?${qs.toString()}`, {credentials:"include"})
              .then(r => r.json());
  } catch(e){
  ...
  if (title) title.textContent =
    `Top ${n} sold-to — ${metric === "qty" ? "Qty" : "Amount"} (2026 YTD)`;
  if (!rows.length) {
  ...
  const rowHtml = rows.map((r, i) => {
    ...
    return `
      <tr class="ts-row${hide}">
        <td>${i + 1}</td>
        <td title="${esc(r.sold_to_name || r.sold_to)}">${esc(r.sold_to_name || r.sold_to)}</td>
        <td>${fmt(metric === "qty" ? r.qty : r.amt)}</td>
      </tr>`;
  }).join("");
  const collapsedCount = showAllInitially ? 0 : (rows.length - 10);
  const expandBtn = collapsedCount > 0
    ? `<button type="button" class="expand-btn" data-tsx="collapsed">
  ...
  body.innerHTML = `
    <table>
      <thead>
        <tr><th>#</th><th>Sold-to</th><th>${metric === "qty" ? "Qty" : "Amount"}</th></tr>
      </thead>
      <tbody>${rowHtml}</tbody>
    </table>
    ${expandBtn}
  ...
  if (btn) {
    btn.addEventListener("click", () => {
      const collapsed = btn.dataset.tsx === "collapsed";
      body.querySelectorAll(".ts-row").forEach((tr, i) => {
