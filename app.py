...
def api_monthly_highlights():
    ...
    try:
        ...
        # ── Context enrichment (Phase 1 holistic analysis) ────────────
        # For each top decliner, attach cheap signals we can compute
        # from data already in the DB.  Every helper is wrapped in a
        # try/except so a missing table (fresh install, or a customer
        # with no visit history) can never break the whole endpoint.
        _PLANT_BY_REGION = {"NSW": "42R1", "QLD": "42R0",
                            "VIC": "42R2", "WA":  "42R4"}
        def _range_bounds(months):
            """Turn a [(y, m), ...] list into a (start_date, end_date_excl)
            pair so SQL can BETWEEN on it consistently."""
            if not months: return (None, None)
            ys, ms = zip(*months)
            y1, m1 = min(months, key=lambda t: (t[0], t[1]))
            y2, m2 = max(months, key=lambda t: (t[0], t[1]))
            start = f"{y1}-{m1:02d}-01"
            if m2 == 12: end = f"{y2 + 1}-01-01"
            else:        end = f"{y2}-{m2 + 1:02d}-01"
            return start, end
        this_start, this_end = _range_bounds(this_months)
        prev_start, prev_end = _range_bounds(prev_months)

        def _pg_context(pg):
            """Signals for a declining product group: stock trend across
            plants, 443 designation, incoming pipeline, open factory PO.
            Everything gracefully degrades to None on error."""
            ctx = {"stock_by_plant": [], "is_443": None,
                   "incoming_qty": None, "factory_po_qty": None}
            try:
                cur.execute(
                    "SELECT s.plant, SUM(s.stock_qty) AS q "
                    "FROM stock s "
                    "JOIN carrying_26 c ON c.m_code = s.material "
                    "WHERE c.product_group = %s "
                    "GROUP BY s.plant ORDER BY s.plant",
                    (pg,),
                )
                ctx["stock_by_plant"] = [
                    {"plant": r["plant"], "qty": int(r["q"] or 0)}
                    for r in cur.fetchall()
                ]
            except Exception: pass
            try:
                cur.execute(
                    "SELECT COUNT(*) AS n FROM `443_25` WHERE product_group = %s",
                    (pg,),
                )
                r = cur.fetchone() or {}
                ctx["is_443"] = int(r.get("n") or 0) > 0
            except Exception: pass
            try:
                cur.execute(
                    "SELECT COALESCE(SUM(i.po_qty), 0) AS q "
                    "FROM incoming i "
                    "JOIN carrying_26 c ON c.m_code = i.material "
                    "WHERE c.product_group = %s",
                    (pg,),
                )
                r = cur.fetchone() or {}
                ctx["incoming_qty"] = int(r.get("q") or 0)
            except Exception: pass
            try:
                cur.execute(
                    "SELECT COALESCE(SUM(o.po_qty), 0) AS q "
                    "FROM orders o "
                    "JOIN carrying_26 c ON c.m_code = o.material "
                    "WHERE c.product_group = %s "
                    "AND o.po_no NOT IN (SELECT DISTINCT po_no FROM incoming "
                    "                    WHERE po_no IS NOT NULL AND TRIM(po_no) <> '')",
                    (pg,),
                )
                r = cur.fetchone() or {}
                ctx["factory_po_qty"] = int(r.get("q") or 0)
            except Exception: pass
            return ctx

        def _customer_context(sold_to):
            """Signals for a declining customer: last visit + gap, meeting
            log activity last 90 days, open claim count, and their prev-
            period top-3 sizes with current national stock so a BDE can
            spot 'they used to buy 205/55R16 which we're now short on'."""
            ctx = {"last_visit": None, "days_since_visit": None,
                   "visits_90d": None, "open_claims": None,
                   "top_sizes_prev": []}
            try:
                cur.execute(
                    "SELECT MAX(visit_date) AS d, COUNT(*) AS n "
                    "FROM meeting_log "
                    "WHERE sold_to = %s "
                    "AND visit_date >= DATE_SUB(CURDATE(), INTERVAL 90 DAY)",
                    (sold_to,),
                )
                r = cur.fetchone() or {}
                if r.get("d"):
                    ...
                    d = r["d"]
                    ctx["last_visit"] = str(d)
                    ctx["days_since_visit"] = (_dt.today() - d).days if hasattr(d, "day") else None
                ctx["visits_90d"] = int(r.get("n") or 0)
            except Exception: pass
            try:
                cur.execute(
                    "SELECT COUNT(*) AS n FROM claim_submission cs "
                    "JOIN customer cu ON cu.ship_to = cs.ship_to "
                    "WHERE cu.sold_to = %s "
                    "AND (cs.status IS NULL OR cs.status NOT IN ('Closed', 'Rejected')) "
                    "AND cs.created_at >= DATE_SUB(CURDATE(), INTERVAL 180 DAY)",
                    (sold_to,),
                )
                r = cur.fetchone() or {}
                ctx["open_claims"] = int(r.get("n") or 0)
            except Exception: pass
            # Top-3 sizes the customer bought in the PREVIOUS period —
            # what they usually want.  Cross-ref with current stock to
            # spot 'BDE, you're low on their favourite size'.
            try:
                if prev_start:
                    cur.execute(
                        "SELECT c.size AS sz, SUM(s.qty) AS q "
                        "FROM sales_2526 s "
                        "JOIN carrying_26 c ON c.m_code = s.material "
                        "WHERE s.sold_to = %s "
                        "AND s.billing_date >= %s AND s.billing_date < %s "
                        "GROUP BY c.size ORDER BY q DESC LIMIT 3",
                        (sold_to, prev_start, prev_end),
                    )
                    rows = cur.fetchall() or []
                    for r in rows:
                        sz = r.get("sz") or ""
                        # National stock for that size (sum across plants)
                        stk = None
                        try:
                            cur.execute(
                                "SELECT SUM(s.stock_qty) AS q "
                                "FROM stock s "
                                "JOIN carrying_26 c ON c.m_code = s.material "
                                "WHERE c.size = %s",
                                (sz,),
                            ...
                            sr = cur.fetchone() or {}
                            stk = int(sr.get("q") or 0)
                        except Exception: pass
                        ctx["top_sizes_prev"].append({
                            "size":       sz,
                            "prev_qty":   int(r.get("q") or 0),
                            "stock_natl": stk,
                        })
            except Exception: pass
            return ctx
        ...
        def _region_context(region):
            """Signals for a declining region: BDE visit-count change,
            active-customer-count change, plant stock, and how many
            steady customers just went silent inside that region."""
            ctx = {"visits_this": None, "visits_prev": None,
                   "active_customers_this": None, "active_customers_prev": None,
                   "plant_stock": None, "plant": _PLANT_BY_REGION.get(region)}
            _STATE_EXPAND = {"NSW": ("NSW", "ACT"), "VIC": ("VIC", "SA", "TAS"),
                             "WA": ("WA", "NT"), "QLD": ("QLD",)}
            members = _STATE_EXPAND.get(region, (region,))
            ph = ",".join(["%s"] * len(members))
            try:
                cur.execute(
                    f"SELECT COUNT(*) AS n FROM meeting_log ml "
                    f"JOIN customer c ON c.ship_to = ml.ship_to "
                    f"WHERE c.bde_state IN ({ph}) "
                    f"AND ml.visit_date >= %s AND ml.visit_date < %s",
                    (*members, this_start, this_end),
                )
                r = cur.fetchone() or {}
                ctx["visits_this"] = int(r.get("n") or 0)
                cur.execute(
                    f"SELECT COUNT(*) AS n FROM meeting_log ml "
                    f"JOIN customer c ON c.ship_to = ml.ship_to "
                    f"WHERE c.bde_state IN ({ph}) "
                    f"AND ml.visit_date >= %s AND ml.visit_date < %s",
                    (*members, prev_start, prev_end),
                )
                r = cur.fetchone() or {}
                ctx["visits_prev"] = int(r.get("n") or 0)
            except Exception: pass
            try:
                cur.execute(
                    f"SELECT COUNT(DISTINCT s.sold_to) AS n FROM sales_2526 s "
                    f"JOIN customer c ON c.ship_to = s.ship_to "
                    f"WHERE c.bde_state IN ({ph}) "
                    f"AND s.qty > 0 "
                    f"AND s.billing_date >= %s AND s.billing_date < %s",
                    (*members, this_start, this_end),
                )
                r = cur.fetchone() or {}
                ctx["active_customers_this"] = int(r.get("n") or 0)
                cur.execute(
                    f"SELECT COUNT(DISTINCT s.sold_to) AS n FROM sales_2526 s "
                    f"JOIN customer c ON c.ship_to = s.ship_to "
                    f"WHERE c.bde_state IN ({ph}) "
                ...
                r = cur.fetchone() or {}
                ctx["active_customers_prev"] = int(r.get("n") or 0)
            except Exception: pass
            try:
                if ctx["plant"]:
                    cur.execute(
                        "SELECT SUM(stock_qty) AS q FROM stock WHERE plant = %s",
                        (ctx["plant"],),
                    )
                    r = cur.fetchone() or {}
                    ctx["plant_stock"] = int(r.get("q") or 0)
            except Exception: pass
            return ctx

        # Attach context to the top-3 decliners in each dimension —
        # small enough that per-item queries stay cheap, but wide
        # enough that a BDE can see the "why" without opening a
        # second tab.  Deeper decliners omit context to keep the
        # response payload bounded.
        for r in region_declines[:3]:
            r["context"] = _region_context(r["region"])
        for r in pg_declines[:3]:
            r["context"] = _pg_context(r["product_group"])
        for r in customer_losses[:3]:
            r["context"] = _customer_context(r["sold_to"])
