...
def get_top_sold_to_from_baseline(cur, f, top_limit, value):
    """Get Top N sold_to based on sales_25_2604 (baseline).
...
def v2_dashboard():
    ...
    try:
        ...
        # ---------------- daily (sales_2604 + target_26 month) ----------------
        ...
        # ---------------- monthly (sales_25_2604 years 2025/2026 + target_26) ----------------
...
def daily_sales():
    ...
    try:
        ...
        # 1) Get Top N sold_to from baseline table (sales_25_2604)
        if top_limit > 0:
...
def export_excel():
    try:
        ...
        filename = f"sales_2604_top{top_limit if top_limit else 'ALL'}_{metric}_{stamp}.xlsx"
...
def yearly_sales():
    ...
    try:
        ...
        # 1) If top_limit > 0, get top N sold_to from baseline (sales_25_2604)
        if top_limit > 0:
            top_sold_to = get_top_sold_to_from_baseline(
                cur, f, top_limit, value
            )  # CHANGED
...
def sold_to_names():
    ...
    try:
        ...
        # Get top sold_to list from baseline table (sales_25_2604)
        top_sold_to = get_top_sold_to_from_baseline(cur, f, top_limit, value)
...
def profit_monthly():
    ...
    try:
        ...
        try:
            ...
            # 1) Top-N sold_to should ALWAYS come from baseline sales table: sales_25_2604
            if top_limit > 0:
